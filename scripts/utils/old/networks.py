...
class SegmentDMPCNN(nn.Module):
    def __init__(self, train_param):
        ...
        self.conv_1             = nn.Conv2d(in_channels=self.model_param.image_dim[0], out_channels=256, kernel_size=10).to(DEVICE)
        self.conv_2             = nn.Conv2d(in_channels=256, out_channels=128, kernel_size=3).to(DEVICE)
        self.conv_width         = nn.Conv2d(in_channels=self.model_param.image_dim[0], out_channels=128, kernel_size=(5, 99)).to(DEVICE)
        self.conv_height        = nn.Conv2d(in_channels=self.model_param.image_dim[0], out_channels=128, kernel_size=(99, 5)).to(DEVICE)
    ...
    def forwardConv(self, x):
        ...
        x1 = F.relu(F.max_pool2d(self.conv_2(x0), 2), inplace=False)
        x1 = flatten(x1, 1)
    ...
    def forward(self, x):
        ...
        tau             = self.output_tau(x).reshape(batch_s, self.max_segments)

        y0 = pos[:, :-1]
        goals = pos[:, 1:] 

        return [num_segments, y0, goals, w, tau]
...
class DSDNetV2(nn.Module):
    def __init__(self, train_param):
        ...
        self.model_param        = self.train_param.model_param
        self.dmp_param          = self.model_param.dmp_param
        self.max_projected_points = self.model_param.max_projected_points
        self.max_segments       = self.model_param.max_segments
        self.dof                = self.dmp_param.dof
        self.n_bf               = self.dmp_param.n_bf
        self.latent_w_size      = self.model_param.latent_w_size
        ...
        self.latent_img_pos     = nn.Linear(self.input_processor_output_size, self.max_projected_points * 2).to(DEVICE)
        self.output_num_segments= nn.Linear(self.max_projected_points * 2, output_num_segments_size).to(DEVICE)
        self.output_y0          = nn.Linear(self.max_projected_points * 2, output_y0_size).to(DEVICE)
        self.output_goal        = nn.Linear(self.max_projected_points * 2, output_goal_size).to(DEVICE)
        # self.output_pos         = nn.Linear(self.max_projected_points * 2, output_pos_size).to(DEVICE)

        self.latent_w           = nn.Linear(self.input_processor_output_size, self.max_segments * self.latent_w_size).to(DEVICE)
        self.output_tau         = nn.Linear(self.latent_w_size, output_tau_size).to(DEVICE)

    def forward(self, x):
        x           = self.input_processor(x)
        batch_s     = x.shape[0]

        latent_img_pos = self.latent_img_pos(x)
        dmp_num_segments = self.output_num_segments(latent_img_pos).reshape(batch_s, 1)
        dmp_y0      = self.output_y0(latent_img_pos).reshape(batch_s, self.max_segments, self.dof)
        dmp_goal    = self.output_goal(latent_img_pos).reshape(batch_s, self.max_segments, self.dof)
        ...
        latent_w    = self.latent_w(x).reshape(batch_s * self.max_segments, self.latent_w_size)
        dmp_weights = self.dmp_weight_decoder(latent_w).reshape(batch_s, self.max_segments, self.dof, self.n_bf)
        dmp_tau     = self.output_tau(latent_w).reshape(batch_s, self.max_segments)
        # dmp_tau     = torch.abs(self.output_tau(x).reshape(batch_s, self.max_segments))

        return [latent_img_pos, dmp_num_segments, dmp_y0, dmp_goal, dmp_weights, dmp_tau]

class SDDMPsTrajLoss(nn.Module):
    def __init__(self, train_param):
        super().__init__()
        self.train_param        = train_param
        self.model_param        = self.train_param.model_param
        ...
        self.dmp                = DMPs_discrete_torch(n_dmps = self.dmp_param.dof,
                                                      ...
                                                      ay = self.dmp_param.ay,
                                                      dt = self.dmp_param.dt)
        self.traj_length        = int(1 / self.dmp_param.dt)
        self.max_segments       = self.model_param.max_segments
        self.dof                = self.dmp_param.dof
        self.n_bf               = self.dmp_param.n_bf
        self.latent_w_size      = self.model_param.latent_w_size
    ...
    def forward(self, x):
        ...
        dmp_num_segments = self.output_num_segments(x).reshape(batch_s, 1)
        dmp_weights = self.dmp_weight_decoder(latent_w).reshape(batch_s, self.max_segments, self.dof, self.n_bf) * 6e4
        # dmp_y0      = self.output_y0(x).reshape(batch_s, self.max_segments, self.dof)
        # dmp_goal    = self.output_goal(x).reshape(batch_s, self.max_segments, self.dof)
        ...
        traj        = zeros(batch_s, self.traj_length * self.max_segments, self.dof).to(DEVICE)
        
        for i in range(self.max_segments):
            self.dmp.w  = dmp_weights[:, i].reshape(batch_s, self.dof, self.n_bf)
            self.dmp.y0 = dmp_goal[:, i].reshape(batch_s, self.dof, 1)
            self.dmp.goal = dmp_y0[:, i].reshape(batch_s, self.dof, 1)
            y, dy, ddy = self.dmp.rollout()
            traj[:, (i * self.traj_length):((i + 1) * self.traj_length)] = y

        return [dmp_num_segments, traj]
...
class PositionNetwork(nn.Module):
    def __init__(self, train_param):
        ...
        input_num_segment_size = 1
        input_position_size = self.dof
        output_position_size = self.dof

        self.input_layer = nn.Bilinear(input_num_segment_size, input_position_size, self.hidden_layer_sizes[0]).to(DEVICE)
        self.output_position = nn.Linear(self.hidden_layer_sizes[-1], output_position_size).to(DEVICE)

    def forward(self, num_segment, in_position):
        x = self.input_layer(num_segment, in_position)
        for fc in self.fc[:-1]:
            x = self.tanh(fc(x))
        x = self.tanh(self.fc[-1](self.dropout(x)))
        output_position = self.output_position(x)
        return [output_position]

class SegmentedDMPNetworkV2(nn.Module):
    def __init__(self, train_param):
        super().__init__()
        self.train_param        = train_param
        self.model_param        = self.train_param.model_param
    ...
    def forward(self, x):
        ...
        return [dmp_num_segments, dmp_y0, dmp_weights, dmp_tau]