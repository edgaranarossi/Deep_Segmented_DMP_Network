...
def generate_shape_segments(shapes,
                            ...
                            dmp_output_ay=None):
    """
    Generate shape segments and save images and data.

    Parameters:
    shapes (list): List of shapes.
    segment_types (list): List of segment types.
    dict_dmp_bf (int): Number of basis functions for DMP.
    dict_dmp_ay (int): Gain term for DMP.
    dict_dmp_dt (float): Time step for DMP.
    subdiv_traj_length (int): Length of subdivided trajectory.
    random_magnitude (float): Magnitude of randomization.
    save_path (str): Path to save images.
    image_target_size (int): Target size of images.
    prefix (str): Prefix for saved images.
    pad_points (int): Number of points to pad.
    dmp_output_dt (float): Output time step for DMP.
    dmp_output_bf (int): Output number of basis functions for DMP.
    dmp_output_ay (int): Output gain term for DMP.
