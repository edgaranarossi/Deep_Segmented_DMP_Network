...
def compare_closest(y_label, y_pred):
    """
    Compare the closest points between the label and prediction.
    
    Parameters:
    y_label (np.ndarray): The label data.
    y_pred (np.ndarray): The prediction data.
...
if __name__ == '__main__':
    ...
    train_param_segment = load_model_parameters(model_dir_segment)
    model_param_segment = train_param_segment.model_param
    dmp_param_segment = model_param_segment.dmp_param
    train_param_deep_dmp = load_model_parameters(model_dir_deep_dmp)
