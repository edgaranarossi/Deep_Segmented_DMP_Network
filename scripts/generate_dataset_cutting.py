...
def generate_cutting_traj(top_left,
                          ...
                          seg_dmp_dt=None):
    """
    Generate cutting trajectory using DMPs.

    Parameters:
    top_left (tuple): Top left coordinates.
    top_right (tuple): Top right coordinates.
    distance (float): Distance between cuts.
    top_padding (float): Padding at the top.
    side_padding (float): Padding at the sides.
    max_segments (int): Maximum number of segments.
    dmp_bf (int): Number of basis functions for DMP.
    dmp_ay (int): Gain term for DMP.
    dmp_dt (float): Time step for DMP.
    dmp_L_bf (int): Number of basis functions for accurate DMP.
    dmp_L_ay (int): Gain term for accurate DMP.
    dmp_L_dt (float): Time step for accurate DMP.
    seg_dmp_bf (int): Number of basis functions for segmented DMP.
    seg_dmp_ay (int): Gain term for segmented DMP.
    seg_dmp_dt (float): Time step for segmented DMP.

    Returns:
    tuple: Generated trajectories and DMPs.
    """
...
if __name__ == '__main__':
    ...
    pkl_data['normal_dmp_L_w'] = np.array(pkl_data['normal_dmp_L_w']).reshape(data_len, 1, dof, pkl_data['normal_dmp_L_bf'])
